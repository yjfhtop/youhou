...
(function () {
     ...
     * @param newMoneyCode
     */
    function getExchangeRate(newMoneyCode = 'CNY') {
        // 放弃了, 懒得使用, 直接用默认值
        // $.ajax({
    ...
    async function getInitData() {
        let rateObj = tryJson(localStorage.getItem(locKeyObj.exchangeRateSaveKey));
        let rateTime = tryJson(localStorage.getItem(locKeyObj.exchangeRateSaveTimeKey));
    ...
    function replaceElPrices(el, unit2ExchangeRateObj, useType = ReplaceElPricesType.innerText, nodeIndex = 0 ) {
        try {
            if (!el) return
            let useTxt = null;
